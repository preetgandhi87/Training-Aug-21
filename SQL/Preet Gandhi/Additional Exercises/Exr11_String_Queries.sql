...
SELECT CONCAT(REPLICATE(0,6),MAX(Salary)) AS 'Max_Sal',CONCAT(REPLICATE(0,6),MIN(Salary)) AS 'Min_Sal' FROM Employees
...
SELECT EmployeeId,FirstName,MONTH(HireDate) AS 'Hire_Month' FROM Employees
...
SELECT FirstName AS "Name",LEN(FirstName) AS 'Length' FROM Employees WHERE FirstName LIKE '[AJM]%' ORDER BY FirstName ASC
