...
SELECT DATEFROMPARTS(DATEPART(yy, DATEADD(mm, -3, GETDATE())), DATEPART(mm, DATEADD(mm, -3, GETDATE())), 01)
...
SELECT DATEDIFF(yy,'2000-08-31',GETDATE())
...
DECLARE @date INT=648
SELECT DATEADD(DAY,@date,'2021-09-11')
...
SELECT DATENAME(YEAR,HireDate) AS 'Years' 
	FROM Employees 
	GROUP BY DATENAME(YEAR,HireDate) 
	HAVING COUNT(EmployeeID) > 10

--17)Write a query to get the department ID, year, and number of employees joined. 
SELECT DepartmentID, DATENAME(YEAR,HireDate) AS YEAR, COUNT(EmployeeID) 'NoOfEmployees' 
	FROM Employees 
	GROUP BY DepartmentID,DATENAME(YEAR,HireDate) 
...
SELECT DepartmentName, FirstName, Salary FROM Departments D JOIN Employees E ON (D.ManagerID=E.EmployeeID) 
...
SELECT FirstName AS 'Name', HireDate,CONCAT(DATEDIFF(dd,HireDate,GETDATE())/365,' ', 'Year',' ',DATEDIFF(MM,HireDate,GETDATE())%12,' ','Month') AS 'Experience' 
	FROM Employees
